...
func Test_Cart(t *testing.T) {
	...
	cmdWithZeroQuantity := &commands.SetCartItemsRequest{
		CartId: args.ID,
	...
	cases := []cartTestCase{{
		...
		expectProductIDsQuery: [][]string{{games[0].Id, games[2].Id}},
		expectGamesQueryResult: [][]*values.Game{{
			gamesMap[games[0].Id],
			gamesMap[games[2].Id],
		}},
		commands: []proto.Message{cmdWithZeroQuantity},
	}, {
		...
		expectProductIDsQuery:  [][]string{{games[0].Id, games[2].Id}},
		expectGamesQueryResult: [][]*values.Game{games},
		commands:               []proto.Message{cmdWithZeroQuantity},
	},
		{
	...
	for _, tc := range cases {
		t.Run(tc.description, func(t *testing.T) {
			...
			actual, err := sut.Calculate(fetcher)
			A.NoError(err)
